...
export function useAuth(options?: UseAuthOptions) {
  ...
  const state = useMemo(() => {
    ...
    // 开发模式：如果不在 Telegram 环境且没有用户，使用模拟用户
    const isDev = typeof window !== 'undefined' && !window.Telegram?.WebApp?.initData;
    const mockUser = isDev && !meQuery.data ? {
      id: 1,
      openId: 'dev-user',
      name: 'Dev User',
      language: 'ru',
    ...
    return {
      ...
      isAuthenticated: Boolean(meQuery.data) || Boolean(mockUser),
    };
  }, [
    isInitializing,
    meQuery.data,
