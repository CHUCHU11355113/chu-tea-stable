...
export async function addUserPoints(userId: number, points: number, reason: string) {
  ...
  // 记录积分历史
  // 获取用户当前积分余额
  const [user] = await db.select({ availablePoints: users.availablePoints }).from(users).where(eq(users.id, userId));
  const newBalance = (user?.availablePoints || 0) + points;
  ...
  await db.insert(pointsHistory).values({
    ...
    type: 'earn',
    balance: newBalance,
    descriptionZh: reason,
    descriptionRu: reason,
